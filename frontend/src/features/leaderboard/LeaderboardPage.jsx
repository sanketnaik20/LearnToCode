...
const generateMockLeaderboard = () => {
    return Array.from({ length: 50 }, (_, i) => ({
        ...
        rank: i + 1,
        username: faker.internet.userName().toLowerCase().replace('_', '.'),
        xp: 5000 - (i * 85) - Math.floor(Math.random() * 50),
        streak: Math.floor(Math.random() * 30),
        isCurrentUser: false,
        status: i < 3 ? 'PRO_LOGICIST' : i < 10 ? 'KERNEL_CONTRIBUTOR' : 'JUNIOR_ARCHITECT'
    }));
};
...
export const LeaderboardPage = () => {
    ...
    useEffect(() => {
        ...
        // Label current user (mocked)
        data[12].isCurrentUser = true;
        data[12].username = "you.admin";
        setUsers(data);
    }, []);
    ...
    return (
        <div className="space-y-12 pb-20 max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                    { label: 'Global Rank', value: '#13', icon: Target },
                    { label: 'Top Percentile', value: 'Upper 2%', icon: TrendingUp },
                    { label: 'Domain Power', value: '4.2k', icon: Zap },
                ].map((stat, i) => (
                    <motion.div 
            ...
            {/* Table */}
            <div className="overflow-hidden border border-brand-grey-100 dark:border-brand-grey-900">
                            ...
                            >
                                ...
                                <td className="p-6">
                                    <div className="flex items-center gap-2">
                                        <Zap size={12} className={user.streak > 15 ? "fill-foreground" : "text-brand-grey-300"} />
